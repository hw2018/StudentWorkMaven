...
java.util.List,
java.util.Iterator,
unp.student.work.manager.domain.PersonFavorite,
unp.student.work.manager.dao.PersonFavoritePageDao,
...
function fun1(id,name,url)
{
	...
	document.getElementById("modifyurl").value = url;
...
public PersonFavoritePageDao personFavoritePageDao = (PersonFavoritePageDao)ctx.getBean("personFavoritePageDao");
public int pageSize;  //用来保存每一页的行数
public int pageCount;	//用来保存页数
public int curPage; 
public PersonFavorite personFavorite;
%>

<%
String studentid = (String)session.getAttribute("studentid"); //获取当前用户的studentid

personFavoritePageDao.initialize(studentid);
pageSize = personFavoritePageDao.getPageSize();  //设置每一页的行数
pageCount = personFavoritePageDao.getPageCount();	//用来保存页数
...
List<PersonFavorite> personFavorites = personFavoritePageDao.getPageResult(curPage,studentid);
if(personFavorites!=null)   //因为一开始可能数据库里没有数据，那么即使传递一个不存在的curPage，它也会查找，结果返回null
{
Iterator it = personFavorites.iterator();
%>
...
<td class="top" style="width:60%;">URL地址</td>
...
do 
{
personFavorite = (PersonFavorite)it.next();  //初始时Iterator的指针指向第一个元素之前，第一个next操作使得它指向第一个元素 
%>
<tr>
<td class="content" style="width:5%;"><%=personFavorite.getId()%></td>
...
<td></td>
<td></td>
<td class="bottom"><a href="<%=basePath%>person/person/person_favorite.jsp?curPage=1">首页&nbsp|</a></td> 
<td class="bottom"><a href="<%=basePath%>person/person/person_favorite.jsp?curPage=<%=curPage-1%>">&nbsp上一页&nbsp|&nbsp</a></td> 
<td class="bottom">第<%=curPage%>页/</td> 
<td class="bottom">共<%=pageCount%>页</td>
<td class="bottom"><a href="<%=basePath%>person/person/person_favorite.jsp?curPage=<%=curPage+1%>">&nbsp|&nbsp下一页</a></td> 
<td class="bottom"><a href="<%=basePath%>person/person/person_favorite.jsp?curPage=<%=pageCount%>">&nbsp|&nbsp尾页</a></td> 
<td ></td>
<td></td>
</tr>
</table>
