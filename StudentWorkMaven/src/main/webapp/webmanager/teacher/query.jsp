...
<html>
  <head>
	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({   
				//url地址改为请求studentquanxianAction
			    url:'${manager}/teacherquanxian/query_teacherquanxian.action',
			    loadMsg:'Loading......',
			    queryParams:{passname:''},//passname参数，这里不需要传具体的passname，因为我们要显示所有的
			    //width:300,
				...
				toolbar: [{
					...
					handler: function(){
						parent.$("#win").window({
							...
							width:350,
							height:200,
							content:'<iframe src="${manager}/send/teacher_save.action" frameborder="0" width="100%" height="100%"/>'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length == 0) {
							...
							parent.$("#win").window({
								...
								width:350,
								height:250,
								content:'<iframe src="${manager}/send/teacher_update.action" frameborder="0" width="100%" height="100%"/>'
							});
							//2. 
						}
					}
						...
						if(rows.length == 0) {
							...
							$.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r){
								if (r){
								    ...
								    //2. 发送ajax请求
								    $.post("${manager}/teacherquanxian/deleteByIds_teacherquanxian.action",{ids:ids},function(result){
								    	if(result == "true") {
			    ...
			    //title：列标题，是显示给人看的
			    columns:[[    		           
			        {field:'passname',title:'账号',width:100, //字段type
			        	//用来格式化当前列的值，返回的是最终的数据
			        	formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
					},  
					{field:'password',title:'密码',width:100, //字段type
			        	//用来格式化当前列的值，返回的是最终的数据
			        	formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
					},   
			        {field:'quanxian',title:'权限',width:100,  //字段hot
						formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
