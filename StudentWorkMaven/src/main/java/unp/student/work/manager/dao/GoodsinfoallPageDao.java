...
@Controller
public class GoodsinfoallPageDao {
	...
	public List<Goodsinfoall> getPageResult(int curPage) {
		if (curPage == 0)
			return null;
		else {
			Session s = sessionFactory.getCurrentSession();
			@SuppressWarnings("unchecked") // 告诉编译器忽略指定的警告，如果没有这一句，编译器会警告funds是unchecked的
			List<Goodsinfoall> favorites = s.createQuery("from Goodsinfoall").setFirstResult((curPage - 1) * pageSize)
